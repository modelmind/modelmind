...
class PersonyEngineV2(Engine[PersonyQuestion]):
    ...
    class Step(StrEnum):
        ...
        @classmethod
        def get_step(cls, question_category: PersonyDimension) -> "PersonyEngineV2.Step":
            ...
            if question_category in PersonyDimension.attitudes():
                return cls.ATTITUDE
            if question_category in PersonyDimension.neuroticism():
                return cls.NEUROTICISM
    ...
    def get_current_step(self, current_result: Result) -> Step:
        ...
        elif counts.get(self.Step.NEUROTICISM, 0) < self.config.questions_count[self.Step.NEUROTICISM]:
            return self.Step.NEUROTICISM
        else:
    ...
    def select_remaining_questions_from_step(
        ...
        elif step == self.Step.NEUROTICISM:
            max_questions = self.config.questions_count[self.Step.NEUROTICISM]
            questions = self._get_neuroticism_questions(current_result, max_questions)
        else:
            questions = []

        # We want to shuffle neuroticism questions across all steps
        max_questions = self.config.questions_count[self.Step.PREFERENCES] // 4
        neuroticism_questions = self._get_neuroticism_questions(current_result, max_questions=6)
        questions += neuroticism_questions
        return questions
    ...
    def _get_neuroticism_questions(
        self, current_result: Result, max_questions: Optional[int] = None
    ) -> List[PersonyQuestion]:
        return self._get_remaining_questions_by_category(PersonyDimension.NEUROTICISM_1, current_result)[:max_questions]
